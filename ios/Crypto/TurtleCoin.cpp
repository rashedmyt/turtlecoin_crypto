...
Native::SecretKey *GenerateViewFromSpend(Native::SecretKey *nativeSpend) {
    ...
    std::copy(std::begin(secretViewKey.data), std::end(secretViewKey.data),
              nativeSecretViewKey->data);

    return nativeSecretViewKey;
}

extern "C" __attribute__((visibility("default"))) __attribute__((used))
bool CheckRingSignatures(
        const Native::Hash *nativePrefixHash,
        const Native::KeyImage *nativeImage,
        Native::PublicKey **nativePubs,
        Native::Signature **nativeSignatures) {
    Crypto::Hash prefixHash;
    Crypto::KeyImage image;

    std::copy(nativePrefixHash->data, nativePrefixHash->data + 32, prefixHash.data);
    std::copy(nativeImage->data, nativeImage->data + 32, image.data);

    std::vector<Crypto::PublicKey> pubs;
    std::vector<Crypto::Signature> signatures;

    while (nativePubs != nullptr) {
        Crypto::PublicKey temp;
        std::copy((*nativePubs)->data, (*nativePubs)->data + 32, temp.data);
        pubs.push_back(temp);
        nativePubs = nativePubs + 32;
    }

    while (nativeSignatures != nullptr) {
        Crypto::Signature temp;
        std::copy((*nativeSignatures)->data, (*nativeSignatures)->data + 64, temp.data);
        signatures.push_back(temp);
        nativeSignatures = nativeSignatures + 64;
    }

    auto success = Crypto::checkRingSignature(prefixHash, image, pubs, signatures);

    return success;
}

extern "C" __attribute__((visibility("default"))) __attribute__((used))
Native::Signature **generateRingSignatures(
        const Native::Hash *nativePrefixHash,
        const Native::KeyImage *nativeKeyImage,
        Native::PublicKey **nativePubs,
        const Native::SecretKey *nativeTxSecretKey) {
    Crypto::Hash prefixHash;
    Crypto::KeyImage keyImage;
    Crypto::SecretKey txSecretKey;

    std::copy(nativePrefixHash->data, nativePrefixHash->data + 32, prefixHash.data);
    std::copy(nativeKeyImage->data, nativeKeyImage->data + 32, keyImage.data);
    std::copy(nativeTxSecretKey->data, nativeTxSecretKey->data + 32, txSecretKey.data);
    ...
    auto success = Crypto::generateRingSignatures(prefixHash, keyImage, pubs, txSecretKey,
                                                  realOutput, signatures);

    if (!success) return nullptr;
    else {
        auto x = signatures.size();

        Native::Signature **res = new Native::Signature *[x];

        for (int i = 0; i < x; i++) {
            auto temp = new Native::Signature();
            std::copy(std::begin(signatures[i].data), std::end(signatures[i].data), temp->data);

            *(res + i * sizeof(Native::Signature *)) = temp;
        }
